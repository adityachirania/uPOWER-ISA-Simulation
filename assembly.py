...
Static_memory = {} # Static memory section of main memory
Instructions = {} #Mapping memory to intructions
...
for line in datalines:
    ...
    if datatype == 'word' : 
        ...
        for w in initialize:
            Static_memory[hex(data_base + static_curr)] = int(w)
...
for line in textlines:
    ...
    if words[0]=="addi":
        ...
        if si<0:
            si = 2**16+si
        binaryterms["si"] = bin(si)[2:].zfill(16)
        binarycode = collectbinary(binaryterms,binarycode)
    ...
    if words[0]=="andi":
        ...
        binaryterms["rs"] = r_num[1]
        binaryterms["ra"] = r_num[0]
        binaryterms["ui"] = bin(int(registers[2]))[2:].zfill(16)
        binarycode = collectbinary(binaryterms,binarycode)
    ...
    if(words[0]=="la"):
        binaryterms = OrderedDict()
        binaryterms["po"] = bin(23)[2:].zfill(6)
        temp=''
        for i in range(1,len(words)):
            temp = temp + words[i]
        a1 = temp.split(",")
        ...
        a2 = a1[1]
        r_num = []
        offset = int(label_lines[a2][2:],16) - data_base
        getnumbers(r_num,registers)
        binaryterms["rt"] = r_num[0]
        binaryterms["off"] = bin(offset)[2:].zfill(20)
        binaryterms["rc"] = "0"
        binarycode = collectbinary(binaryterms,binarycode)
...
#########################################################################
    ...
    if words[0] == "b":
        ...
        if li<0:
            li = 2**24 + li
        binaryterms["li"] = bin(li)[2:].zfill(24)
        binaryterms["aa"] = '0'
        binaryterms["lk"] = '0'
        binarycode = collectbinary(binaryterms,binarycode)
    ...
    if words[0] == "bc":
        ...
        for i in range(1,len(words)):
            temp = temp + words[i]
        registers = temp.split(",")
        offset  = int(label_lines[registers[2]],16) - current_addr
        print(offset)
        if offset<0:
            offset = 2**14+offset
        binaryterms['bo'] = bin(int(registers[0]))[2:].zfill(5)
        binaryterms['bi'] = bin(int(registers[1]))[2:].zfill(5)
        binaryterms["bd"] = bin(offset)[2:].zfill(14)
        print(binaryterms["bd"])
        binaryterms["aa"] = "0"
        binaryterms["lk"] = "0"
        binarycode = collectbinary(binaryterms,binarycode)
    ...
    if words[0]=="cmp":
        ...
        r_num = []
        getnumbers(r_num,registers[2:])
        binaryterms["bf"] = bin(7)[2:].zfill(3)
        binaryterms["l"]  = bin(1)[2:].zfill(2) 
        binaryterms["ra"] = r_num[0]
        binaryterms["rb"] = r_num[1]
        binaryterms["xo"] = bin(0)[2:].zfill(10)
        binaryterms["rc"] = '0'
        binarycode = collectbinary(binaryterms,binarycode)
    ...
    if words[0]=="cmpi":
        ...
        if si<0:
            si = 2**16+si
        getnumbers(r_num,registers[2:3])
        binaryterms["bf"] = bin(7)[2:].zfill(3)
        binaryterms["l"]  = bin(1)[2:].zfill(2)
        binaryterms["ra"] = r_num[0]
        binaryterms["si"] = bin(si)[2:].zfill(16)
    ...
    # Checking whether system call
    if words[0]=="syscall":
        binarycode = bin(0)[2:].zfill(32)
        

    print(binarycode)
    ######################
    
    myfile1.write(binarycode+"\n")
    Instructions[hex(current_addr)] = binarycode
